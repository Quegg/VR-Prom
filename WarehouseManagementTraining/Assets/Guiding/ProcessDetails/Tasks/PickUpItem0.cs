...
{
    public class PickUpItem0 : MonoBehaviour, ITaskDetails {
        private GuidingController myGuidingController;
        private string classNameRaw="PickUpItem0";
        private Stock.Stock stock;
        private OrderManager orderManager;
        private List<OutlineController> smallItemOutlinesOfCurrentOrder;
        ...
        private void Start()
        {
            smallItemOutlinesOfCurrentOrder=new List<OutlineController>();
        }
        ...
        public void ShowHelp()
        {
            ...
            foreach (var neededItem in orderManager.ItemsNeededInCurrentOrder())
            {
                if (neededItem.Key is SmallItem)
                {
                    //cache outlineControllers, so we do not need to search again for deactivating
                    smallItemOutlinesOfCurrentOrder.AddRange(stock.SmallItemOutlines[neededItem.Key.id]);
                }
            }

            foreach (var itemOutline in smallItemOutlinesOfCurrentOrder)
            {
        ...
        private void HideHelp()
        {
            helpShown = false;
            mistakesHappened = 0;
            foreach (var outline in smallItemOutlinesOfCurrentOrder)
            {
                outline.HideOutline();
            }
            smallItemOutlinesOfCurrentOrder.Clear();
        }
